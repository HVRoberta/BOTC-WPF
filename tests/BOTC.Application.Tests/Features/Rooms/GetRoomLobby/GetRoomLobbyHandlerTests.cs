...
public sealed class GetRoomLobbyHandlerTests
{
    ...
    public async Task HandleAsync_WhenRoomExists_ReturnsLobbyState()
    {
        ...
        repository.SeedLobby(new GetRoomLobbyResult(
            new RoomCode("AB12CD"),
            [
                new LobbyPlayerResult(new PlayerId(Guid.NewGuid()), "Host", PlayerRole.Host, false),
                new LobbyPlayerResult(new PlayerId(Guid.NewGuid()), "Alice", PlayerRole.Player, false)
            ],
